...
_Add your deployed URL here once live, e.g. https://your-app.onrender.com/docs_
...
1. Sign up at [neon.com](https://neon.com) (no credit card needed).
2. Create a new project — Neon creates a default database automatically.
...
Works on Render or Vercel — since state now lives in Neon rather than in memory, the persistence caveats from Project 2 no longer apply here. Whichever platform you use:

1. Push this folder to its own GitHub repository.
2. Connect that repo to your hosting platform.
3. **Add `DATABASE_URL` as an environment variable in the platform's dashboard** — do not commit your real `.env` file (it's gitignored on purpose).
4. Build/start commands (Render): Build = `pip install -r requirements.txt`, Start = `uvicorn main:app --host 0.0.0.0 --port $PORT`

## Notes

- Neon's free tier auto-suspends after inactivity and wakes automatically on the next request — expect the first request after a quiet period to take a couple of seconds longer, then run normally.
- `PUT` replaces a city's data entirely (including the full forecast) rather than patching individual fields — keeps the validation logic simple and predictable.
